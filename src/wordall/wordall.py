...
class Game(abc.ABC):
    """
    Abstract base class for word-guessing game logic. Does not handle any user interaction.
    In general, will accept guesses until the correct word(s) is found, at which point the game ends.
    """
    ...
    def guess_word(self, guess_word: str) -> bool:
        ...
        Returns:
            True if the game is "complete", False if the game is not complete. The game is complete when the user has guessed the target word or words successfully.
        """
        pass
...
class WordleGame(Game):
    ...
    def _load_word_list(self, word_list_path: str) -> list[str]:
        """
        Loads the words for the given file and returns them as a list.
        The words in the file should be one per line.
        Raises InvalidWordListWordError if any word does not match the alphabet.
        """
    ...
    def _select_target(self) -> str:
        """
        Chooses a target word, which the user must try to guess, randomly from the word list.
        """
